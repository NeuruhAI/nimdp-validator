...
def aggregate_scores(texts: List[str], token_map: Dict[str, Any]) -> Dict[str, Any]:
    ...
    results = {"phases": {}, "hard_block_triggered": False, "score": 0.0}
    total_score = 0.0
    remediations = []
    for pname, pdef in phases.items():
        pscore, tokens, fixes, hb = score_phase(joined, pname, pdef)
        results["phases"][pname] = {"score_contrib": pscore, "tokens": tokens}
        total_score += pscore
        if hb:
...
def write_reports(project_name: str, outdir: Path, details: Dict[str, Any]):
    ts = now_stamp()
...
def push_notion(project_name: str, details: Dict[str, Any]) -> Dict[str, Any]:
    ...
    payload = {
        ...
        "properties": {
            ...
            "Timestamp (UTC)": {"date": {"start": dt.datetime.utcnow().isoformat()}}
        },
        "children": [{
            "object": "block",
            "type": "paragraph",
